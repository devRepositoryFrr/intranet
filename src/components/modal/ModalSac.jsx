...
export default function ModalSac() {
	...
	const validarcmb = (id, require) => {

		if (id == "ingresa_vehiculo") {
			mostrarHijo(id, $("#" + id).val());
		}
		var do_ = true;
	...
	const submitInsert = () => {
		const d = new Date($("#fecha_visita").val());
		const formatter = new Intl.DateTimeFormat('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
		const formattedTime = formatter.format(d);

		//console.log(d.toLocaleDateString() + " " + formattedTime.toString());
		let _fecha = d.toLocaleDateString() + " " + formattedTime.toString();


	

		if (validaCampos()) {
			Axios.post(url + '/api/solicitar_acceso', {
				...
				ingresa_vehiculo: $("#ingresa_vehiculo").val(),
				marca: $("#datos_marca").val(),
				submarca: $("#datos_submarca").val(),
				placas: $("#datos_placas").val(),
				color: $("#datos_color").val(),
				...
				(response) => {
					...
					if (response.data != "") {
						var vehiculo = ""
						if ($("#ingresa_vehiculo").val() == "SI") {
							vehiculo = "<p><span style='color:#821b3f'>●Marca: </span><strong>" + $("#datos_marca").val() + "</strong></p>" +
								"<p><span style='color:#821b3f'>●Submarca: </span><strong>" + $("#datos_submarca").val() + "</strong></p>" +
								"<p><span style='color:#821b3f'>●Placas: </span><strong>" + $("#datos_placas").val() + "</strong></p>" +
								"<p><span style='color:#821b3f'>●Color: </span><strong>" + $("#datos_color").val() + "</strong></p>"
								;
						} else {
						...
						/*cuerpo*/"<p><span style='color:#821b3f'>●Nombre del solicitante:</span><strong>" + user[0].nombre +  "</strong></p>" +
						"<p><span style='color:#821b3f'></strong> ●Nombre de la persona que ingresa:</span><strong>" + $("#nombre_visitante").val() +"</strong></p>"+
						 "</strong> Si necesitas más detalles puedes escribirle al solicitante a:" +
						...
						/*asunto*/"<p style='text-align:center'><strong>**Datos adicionales**</strong></p>",
							...
							"<p><span style='color:#821b3f'>●Fecha y hora de la visita: </span><strong>" + _fecha + " </strong></p>" +
							...
							"<p><span style='color:#821b3f'>●Ingresa con vehiculo: </span><strong>" + $("#ingresa_vehiculo").val() + "</strong></p>" +
							"<p style='margin-top:20px'>Datos del vehiculo:<p>" +
							vehiculo,
		...
		<div id='content-sac' class='modal-content' style={{ display: "none", overflow: "hidden" }}>
			...
			<div class='modal-body' style={{ height: 450, overflowX: "auto", overflowY: "auto" }}>
				...
				<div class="text-justify">
					<select class="form-control" name="ingresa_vehiculo" id="ingresa_vehiculo"
						onChange={(e) => validarcmb(e.target.id)}>
						<option value="0" >--SELECCIONA--</option>
						<option value="SI">Si (esta sujeto a disponibilidad de espacios de estacionamiento)</option>
						<option value="NO">No</option>
					</select>

					<label id="ingresa_vehiculo-errorSAC" class="errorSAC_s errorSAC" for="ingresa_vehiculo" style={{ display: 'none' }}>Campo requerido *</label>
				</div>

				<div class="ingresa_vehiculo hidden">
					<p class="top-buffer-x25" >Datos del vehículo (Marca, submarca, placas y color):</p>
					<div>
						<label style={{ fontSize: 12 }}>Marca: <span class="crt-count" style={{ fontSize: 12 }}>150</span></label>
						<textarea maxLength="150" minLength="1"
							id="datos_marca"
						...
						<label id="datos_marca-errorSAC" class="errorSAC_s errorSAC ingresa_vehiculo hidden" for="datos_marca" >Campo requerido *</label>
					</div>
					<div>
						<label style={{ fontSize: 12 }}>Submarca: <span class="crt-count" style={{ fontSize: 12 }}>150</span></label>
						<textarea maxLength="150" minLength="1"
							id="datos_submarca"
							placeholder="Especifica la submarca"
							style={{ resize: "none" }} class='form-control  max text'
							onChange={(e) => validar(e.target.id, e.target.minLength)}
						>
						</textarea>
						<label id="datos_submarca-errorSAC" class="errorSAC_s errorSAC ingresa_vehiculo hidden" for="datos_submarca" >Campo requerido *</label>
					</div>
					<div>
						<label style={{ fontSize: 12 }}>Placas: <span class="crt-count" style={{ fontSize: 12 }}>150</span></label>
						<textarea maxLength="150" minLength="1"
							id="datos_placas"
							placeholder="Especifica las placas"
							style={{ resize: "none" }} class='form-control  max text'
							onChange={(e) => validar(e.target.id, e.target.minLength)}
						>
						</textarea>
						<label id="datos_placas-errorSAC" class="errorSAC_s errorSAC ingresa_vehiculo hidden" for="datos_placas" >Campo requerido *</label>
					</div>
					<div>
						<label style={{ fontSize: 12 }}>Color: <span class="crt-count" style={{ fontSize: 12 }}>150</span></label>
						<textarea maxLength="150" minLength="1"
							id="datos_color"
						...
						<label id="datos_color-errorSAC" class="errorSAC_s errorSAC ingresa_vehiculo hidden" for="datos_color" >Campo requerido *</label>
					</div>
				</div>
