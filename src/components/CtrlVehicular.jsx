...
function tabla(json) {
	dt = $("#tblAVehiculo").DataTable({
		...
		//	{ data: 'Folio'},
			...
			{ data: 'Email'},
			{ data: 'Nombre'},
		//	{ data: 'Cargo del solicitante'},
		//	{ data: 'Area_del_solicitante'},
			{ data: 'Telefono_del_solicitante'},
			{ data: 'Descripcion_de_la_ruta'},
		//	{ data: 'Numero_de_personas'},
		//	{ data: 'Personal_de_manejo'},
...
function getCtrlVehicular() {
	var user = JSON.parse(localStorage.getItem('credenciales'));
	let mes = $("#cmbMes").val();
	//console.log("acceso")
	//console.log(mes)

	Axios.get(api_cnfg + "/api/CtrlVehicular").then((res) => {
		json = res.data[0];
...
function CtrlVehicular(props) {
	...
	const limpiar = () => {
		...
		$('#tblAVehiculo').DataTable().clear().destroy();
	}
			...
			<header>
				...
				<div id="cont" class="container">
					...
					<ol id="bread" class="breadcrumb">
						...
						<li class="active"><Link class="nav-link">Control de Acceso vehicular</Link></li>
					</ol>
					...
					<div class="col-md-6">
						<div class="form-group">
							...
							<select class="input-sm form-control" id="cmbMes" onChange={(e) => chMes(e.target.id)}>
					...
					<a href='https://sistemaintegral.conavi.gob.mx:81/api/rep_materiales/sp_get_Svehiculo' target={"_blank"} type='button' ><i class="fa fa-download btn btn-primary pull-right " aria-hidden="true" ></i></a>


					<table class="table-bordered table-detail  center " id="tblAVehiculo" style={{ width: "100%" }} >
						<thead>
							<tr class="">
