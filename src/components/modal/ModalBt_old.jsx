...
export default function ModalBt() {
	...
	let subject = "soportesistemas@conavi.gob.mx;mmcasillas@conavi.gob.mx;"+ $("#cboUsuario").children("option:selected").val();
			...
			Axios.post('https://sistemaintegral.conavi.gob.mx:81/api/solicitar_sala', {
				...
				(response) => {
					...
					if (response.data != "") {
						...
						/*cuerpo*/"Nueva solicitud de <strong>" + user[0].nombre + "</strong> para la <strong>" + $("#cboSala").children("option:selected").text() + "</strong> el día <strong>"+moment(dbfInicial).format('YYYY-MM-DD')+" de "+$("#cboHinicio").children("option:selected").text()+ " a "+$("#cboHtermino").children("option:selected").text()+" </strong>, con capacidad para <strong>" + $("#asistentes").val() + "</strong> personas, donde será celebrada la reunión '<strong>" + $("#titulo_evento").val() + "</strong>', con integrantes de <strong>" + $("#institucion").val() + "</strong>. Si necesitas más detalles puedes escribirle al solicitante a:" +
	...
	return (
		<div id='content-bt' class='modal-content' style={{ display: "none", overflow: "hidden" }}>
			...
			<div class='modal-body' style={{ height: 450, overflowX: "auto", overflowY: "auto" }}>
				...
				<p class="top-buffer-x25">Salas:</p>
				<select onChange={verHorarios} class="form-control" id="cboSala">
					<option value="0">--Selecciona una sala--</option>
					<option value="1">Sala de consejo</option>
					<option value="2">Sala piso 2 oriente</option>
					<option value="3">Sala arq. Granados piso 2</option>
					<option value="4">Sala de administración</option>
					<option value="5">Sala de videoconferencia (sujeta a disponibilidad)</option>
				</select>
				<label id="cboSala-errorBT" class="error" for="cboSala" >Campo requerido *</label>
